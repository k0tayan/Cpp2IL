        ...
        foreach (var va in possibleMetadataUsages)
        {
            try
            {
                ...
                for (var i = 0; i < mrWords.Length && ok; i++)
                {
                    ...
                    {
                        //Count
                        ok = mrWords[i] < 0x200000;

                        if (!ok)
                            LibLogger.VerboseNewline($"\t\t\tRejected Metadata registration at 0x{va:X}, because it has a count field 0x{mrWords[i]:X} at offset {i} which is above sanity limit of 0x200000. If metadata registration detection fails, may need to bump up the limit.");
                    }
                    else
                    {
                        //Pointer
